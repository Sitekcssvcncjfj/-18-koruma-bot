...
async def nsfw_kontrol(file_id: str, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    Dikkat: video için file_url doğru oluşturulmalı ve zaman aşımı dikkate alınır.
    """
    try:
...
.0/check.json", params=params)
            data = r.json()

        if data.get("status") == "success":
            n = data.get("nudity", {})
            # Yüksek hassasiyetli NSFW ölçütleri (aşırı erotik/çıplaklık seviyeleri)
            return (
...
async def komut_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Yöneticiler kontroladministrator", "creator"):
            return
    except Exception as e:
    ...
    try:
        print(f"Medya türü tespit edildi: {msg.effective_attachment}")  # DEBUG: hangi tür, context):
                await msg.delete()
                print(f"Video silindi (NSFW) | Kullanıcı: {msg.from_user.username or msg.from_user.id}")
        elif msg.sticker:
            # Video/tgs/webm gibi hareketli sticker’ları risk olarak direkt silebiliriz; 
            # veya Sightengine’le tarama yapmak da mümkün. Burada “önce tara” seçeneğini tercih ettik:
        ...
        print(f"Medya silme/analiz hat print("Bot Başlatıldı... Loglar aktif ( tüm medya için silme hazır).")
